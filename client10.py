import socket
import os
from scapy.all import rdpcap, IP,TCP,Padding,Raw
import argparse
import sys
...
def pcap_file():
    parser = argparse.ArgumentParser(description="處理 PCAP 檔案的 Client 程式")
    parser.add_argument("pcap_file", help="請輸入要執行的 pcap 檔案路徑")
    parser.add_argument("--ip", required=True, help="請輸入 Client 的 IPv4 位址")
    # 解析命令列參數
    args = parser.parse_args()
    target_pcap = args.pcap_file
    client_ip = args.ip
    
    print(f"取得檔案位置：{target_pcap}")
    return target_pcap, client_ip
...
def playback(sock, target_file, CLIENT_IP):
    ...
    while i < total:
        ...
        if pkt.haslayer(Padding) and len(pkt[Padding].load) > 0:
            i+=1
            continue
        ...
        if pkt[IP].src == CLIENT_IP:
            ...
            transport_header = bytes(pkt[TCP].payload)[:12].hex()
        ...
        # 等對方回應 (Src 是伺服器，且 Dst 是我)
            ...
            print(f"<-成功接收回應 [{i+1}] ({len(received_data)} bytes) | Payload: 0x{received_data[:12].hex()}")
